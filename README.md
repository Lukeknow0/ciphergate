...
> **Current status (2026-07-18): submitted to DoraHacks and under review.** CipherGate is deployed at `0xe0df8484d6986e1ef9b4ef04a263d72708560b71`. Intent #0 reached proof-published `PASS`, its exact checksummed batch imported successfully into Safe Transaction Builder, and no Safe proposal was signed or executed. The public source repository, Pages frontend, CI run, `v1.0.0-hackathon` demo release, and required [X post with the 3:32 demo](https://x.com/mr_luhs/status/2078331526593577406) are live. The [DoraHacks BUIDL](https://dorahacks.io/buidl/47145) is submitted and the organizer currently shows it as `Under Review`; its submission list is organizer-private. The separate official Nox Hello World deployment remains onboarding evidence only.
...
- `frontend/` uses the official Nox handle client and viem for wallet connection, three handle/proof requests, submission, encrypted evaluation, public-decision proof retrieval/publication, and PASS-only Safe batch export. Both the fail-closed unconfigured preview and a production build configured with the live CipherGate address have been exercised; the live production flow reached PASS and exported the imported batch recorded in E-015/E-016.
...
scripts/     Build, policy/ABI/doc checks, plus Sepolia deploy and read-only smoke helpers
...
`npm run build:frontend` is the release build and requires `CIPHERGATE_CONTRACT_ADDRESS` to be a non-zero deployed address. `npm run test:nox` starts the official Nox Docker stack and performs the real encryption/decryption, ACL, proof, replay, publication, boundary, and exact-action tests; it is intentionally separate from the fast checks. The recorded run exits zero with two checkmarks and `2 passing (2 nodejs)`.

### Verification snapshot

| Check | Result through 2026-07-18 | Evidence |
|---|---|---|
...
| Public source, CI, Pages, demo release, X post, and DoraHacks submission | PASS: [repository](https://github.com/Lukeknow0/ciphergate), [CI](https://github.com/Lukeknow0/ciphergate/actions/runs/29630254354), [Pages](https://lukeknow0.github.io/ciphergate/), [release](https://github.com/Lukeknow0/ciphergate/releases/tag/v1.0.0-hackathon), [X post](https://x.com/mr_luhs/status/2078331526593577406), [BUIDL](https://dorahacks.io/buidl/47145) `Under Review` | [E-013](EVIDENCE.md#e-013--source-control-and-release-evidence) |
...
The encrypted deposit receipt has `status: 0x1` and targets the canonical `0x372Be2…B2EC0` contract. After the three onboarding transactions the wallet nonce was `3`; later CipherGate and Safe transactions naturally advanced it. The red MetaMask “failed” item was local activity history rather than an additional failed on-chain transaction. These onboarding transactions do **not** substitute for the separate CipherGate deployment and live-flow receipts in E-012/E-015.
...
The public repository, CI run, Pages frontend, video release, required X post, and DoraHacks BUIDL submission are available above. The organizer currently lists submissions privately and marks CipherGate `Under Review`. The Safe JSON import was validation-only; no Safe signature or execution occurred.
