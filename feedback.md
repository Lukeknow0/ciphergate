...
- During earlier failed starts, the surfaced exception was `Error: [nox] Failed to start the offchain stack: [object Object]`. The nested failure was not serialized in the Hardhat error.
- Manual `docker compose ... config` validation succeeded, and the official Nox `0.6.0` tags exposed `linux/arm64` manifests. Initial system-proxy runs exposed HTTP 503/truncated image pulls through the local proxy. A controlled `No proxy` plus full Docker restart retry reached Docker Registry directly but failed while acquiring `iexechub/nox-ingestor:0.6.0`; a read-only manifest inspection reproduced a TLS hostname mismatch in which `registry-1.docker.io` presented an unrelated AWS Lambda certificate. At that historical point no Nox containers or images remained and the plugin log was empty. The two local network paths failed differently, while the plugin obscured the actionable nested Docker/Compose error as `[object Object]`.
- A later run succeeded after manually configuring Docker Desktop's Containers proxy and pulling the official images. A fresh `npm run test:nox` exited `0`, printed two `✔` cases, and ended with `2 passing (2 nodejs)`. The passing coverage includes real wallet/contract proof binding, confidential-attribute and ACL negatives, malformed/cross-intent/replay rejection, one-time evaluation/publication, proof-bound public decisions, exact/mismatched action gating, and six strict Solidity boundaries.
- Hardhat's shared-RPC wallet clients return all accounts from `getAddresses()`, while `@iexec-nox/handle` selects the first. Binding the test submitter client's result to `[submitter.account.address]` was required to align the handle signer with the transaction signer. This is an account-selection adapter for the test environment, not a mock of encryption, proofs, ACLs, or product behavior.
- After the successful run, Containers proxy was restored to `Same as host proxy`, Docker Desktop was fully restarted, and UI/CLI checks confirmed `Engine running` with no residual Nox containers. The earlier network failures are resolved for the current run, but the plugin's `[object Object]` error still hid actionable diagnostics and remains worth improving.
