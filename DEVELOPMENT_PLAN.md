...
Current gate status (2026-07-17): the official Hello World happy path and live ACL-producing deposit passed. CipherGate compiles; browser-adapter, ACL/metadata/commitment, policy-vector, Safe-adapter, public-surface, documentation, explicit unconfigured preview-build, and unconfigured desktop/mobile browser QA checks pass. A fresh `npm run test:nox` also started the real official Nox Docker stack and exited `0` with two `✔` cases and `2 passing (2 nodejs)`. Its first case covers wallet/contract proof binding, private-attribute confidentiality and ACL negatives, malformed/cross-intent/replay rejection, repeated evaluation/publication rejection, proof-bound public decisions, and exact/mismatched action gating; its second case covers all six strict Solidity comparison boundaries. Because Hardhat shared-RPC wallet clients return every account from `getAddresses()` while `@iexec-nox/handle` chooses the first, the test binds the submitter client's result to `submitter.account.address`; this is test account selection, not a Nox cryptographic or product mock. The official images are now pulled. The temporary manual Containers proxy configuration used to complete the operation was restored to `Same as host proxy`, followed by a full Docker restart and UI/CLI verification of `Engine running` with no leftover Nox containers. Earlier proxy/DNS/TLS failures remain useful troubleshooting history but are no longer the current gate blocker. The exact current revision also passed an official npm-registry `npm ci` and complete `npm run check` from an empty isolated copy; all 202/202 lockfile dependency entries contain `resolved` and `integrity`. The online npm audit reports 16 development/transitive findings (0 critical, 2 high, 6 moderate, 8 low) through the pinned Nox/Hardhat path, with no compatible direct fix; no blanket fix was applied. The project is still not submission-ready: CipherGate has no Sepolia deployment, the live product browser flow and actual Safe JSON import have not run, and no public release exists. The separate Hello World Sepolia address is onboarding evidence, not a CipherGate address. A reviewed local source commit exists, but no release tag exists. See `STATUS.md` and `EVIDENCE.md` for the authoritative snapshot.
